...
interface EditProfileModalProps {
  ...
  const EditProfileModal:React.FC<EditProfileModalProps> = ({isOpen,onClose,doctorData,onProfileUpdate}) => {

    // console.log(">>>>>>>>",doctorData);
    

    const [imagePreview,setImagePreview] = useState<string|null>(doctorData.profileImage || null)
   ...
   const handleImageUpload = (event:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    const uploadToCloudinary = async (file:string|File):Promise<string|null>=>{
        ...
        <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-6">
          ...
          <div className="flex justify-center mb-6">
            <label className="cursor-pointer flex flex-col items-center">
              ...
              <span className="mt-2 text-sm text-gray-500">Upload New Picture</span>
            </label>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <input 
                type="number" 
                {...register("experience", { required: "Experience is required", min: { value: 1, message: "Must be at least 1 year" } })} 
                className="w-full p-2 border border-gray-300 rounded-md focus:ring focus:ring-blue-300"
              />
            ...
            <div className="flex justify-end space-x-3">
              <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700">
                Cancel
              </button>
              <button type="submit" className="px-4 py-2 bg-blue-900 text-white rounded-md hover:bg-blue-700" disabled={isSubmitting}>
