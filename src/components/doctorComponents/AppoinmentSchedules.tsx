...
import { Calendar, ChevronLeft, ChevronRight, Filter, Eye, Search, X, Loader2, CalendarDaysIcon } from 'lucide-react';
import DatePicker from 'react-datepicker';
import "react-datepicker/dist/react-datepicker.css";
import { fetchSchedules, getServices } from '@/services/doctor/doctorService';
...
interface Slot {
  slot_id: string;
  status: 'available' | 'booked' | 'pending' | 'rescheduled' | 'cancelled' | 'completed';
  start_time: string;
  end_time: string;
  is_break: boolean;
  _id: string;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading && (
          <div className="flex justify-center items-center py-12">
            ...
            <span className="ml-2 text-lg font-medium text-gray-700">Loading schedules...</span>
          </div>
        )}
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex justify-between text-sm mt-2">
                      ...
                      <span className="font-medium text-gray-900">{schedule.serviceId.mode}</span>
                    </div>
                    <div className="flex justify-between text-sm mt-2">
                      <span className="text-gray-500">Fee</span>
                      <span className="font-medium text-gray-900">₹{schedule.serviceId.fee}</span>
                    </div>
                    <div className="flex justify-between text-sm mt-2">
                    ...
                    <div className="flex justify-between text-sm mt-2">
                      ...
                      <span className="font-medium text-gray-900">{schedule.duration} mins</span>
                    </div>
                    <div className="flex justify-between text-sm mt-2">
                      <span className="text-gray-500">Available Slots</span>
                      <span className={`font-medium ${countAvailableSlots(schedule.availability) > 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {countAvailableSlots(schedule.availability)} / {schedule.availability.length}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">
                    {Math.min(pagination.currentPage * filters.limit, pagination.totalRecords)}
                  </span>{' '}
            ...
            <div className="px-6 py-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Doctor ID</h4>
                    <p className="text-base font-medium text-gray-900">{selectedSchedule.doctorId}</p>
                    {selectedSchedule.doctor && (
                      <p className="text-sm text-gray-500">{selectedSchedule.doctor.specialization}</p>
                    )}
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Service</h4>
                    <p className="text-base font-medium text-gray-900">{selectedSchedule.serviceId.name}</p>
                    <p className="text-sm text-gray-500">Mode: {selectedSchedule.serviceId.mode}</p>
                    <p className="text-sm text-gray-500">Fee: ₹{selectedSchedule.serviceId.fee}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-gray-500">Date</h4>
                    <p className="text-base font-medium text-gray-900">{formatDate(selectedSchedule.date)}</p>
                  </div>
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Duration: {selectedSchedule.duration} minutes</p>
                  </div>
                </div>
              </div>
              
              {/* Availability Slots */}
              <h4 className="text-lg font-medium text-gray-900 mb-3">Availability Slots</h4>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {selectedSchedule.availability.map((slot) => (
                  <div
                    key={slot._id}
                    className={`p-3 rounded-lg flex items-center ${getStatusColor(slot.status)}`}
                  >
                    <div className="mr-2">
                    ...
                    <div>
                      ...
                      {slot.is_break && <p className="text-xs font-medium text-gray-500">Break Time</p>}
                      {slot.patient && <p className="text-xs">{slot.patient}</p>}
                    </div>
                  </div>
                ))}
              </div>
