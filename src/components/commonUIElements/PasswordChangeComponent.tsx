...
const PasswordChangeComponent: React.FC<PasswordChangeComponentProps> = ({
  ...
  const themeColorClasses = userType === 'doctor' 
    ? 'text-[#02045e] border-[#02045e] bg-[#02045e] ring-[#02045e]/30' 
    : 'text-[#04bf3e] border-[#04bf3e] bg-[#04bf3e] ring-[#04bf3e]/30';
  
  const [colorText, colorBorder, colorBg, colorRing] = themeColorClasses.split(' ');
  ...
  const onFormSubmit = async (data: PasswordChangeFormData) => {
    try {
        ...
        const response = await changePassword(id,passwordData.currentPassword,passwordData.newPassword)
    ...
    <div className="w-full max-w-md mx-auto bg-white rounded-lg shadow-md p-6">
      ...
      <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
        ...
        <div className="space-y-1">
          <label htmlFor="currentPassword" className={`block text-sm font-medium ${colorText}`}>
            Current Password
          </label>
        ...
        {/* New Password Field */}
        <div className="space-y-1">
          <label htmlFor="newPassword" className={`block text-sm font-medium ${colorText}`}>
            New Password
          </label>
          <div className="relative">
            <input
        ...
        <div className="space-y-1">
          <label htmlFor="confirmPassword" className={`block text-sm font-medium ${colorText}`}>
        ...
        <button
          ...
          className={`w-full flex items-center justify-center text-white py-2 px-4 rounded-md focus:outline-none focus:ring-2 transition-colors ${colorBg} focus:${colorRing} ${isSubmitting ? 'opacity-70' : 'opacity-100'}`}
        >
          {isSubmitting ? (
